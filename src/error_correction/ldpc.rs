...
#[cfg(test)]
mod tests {
    use crate::{error_correction::{checksum::checksum, gray::GrayCode}, message::Message};
    ...
    fn extract_parity_bits_from_symbols_str(symbols_str: &str) -> u128 {
    ...
    #[test]
    fn from_message() {
        let message = Message::try_from("CQ SOTA N0YPR/R DM42").unwrap().message;
        
        let crc = checksum(message);

        let parity = generate_parity(message, crc);

        let expected_parity = extract_parity_bits_from_symbols_str("3140652000671215006116571652175530543140652375421655752603157715414212433140652");
