...
fn test_wsjtx_minus15db_signal() {
    ...
    // WSJT-X reports SNR: -15 dB for this signal
    ...
    let found = decoded_messages.iter().any(|m| m.message == expected);
    assert!(found, "Should decode expected message: '{}'", expected);
    
    // Check SNR is reasonable (within a few dB of WSJT-X's -15 dB)
    let decoded = decoded_messages.iter().find(|m| m.message == expected).unwrap();
    println!("\n✓ Decoded '{}' at SNR {} dB", decoded.message, decoded.snr_db);
    println!("  WSJT-X reported: -15 dB");
    println!("  Difference: {} dB", decoded.snr_db - (-15));
    
    // Allow ±5 dB tolerance for now
    assert!(decoded.snr_db >= -20 && decoded.snr_db <= -10,
        "SNR {} dB should be close to WSJT-X's -15 dB (±5 dB tolerance)", decoded.snr_db);
}
